...
- **Constellation scheduling (implemented):** every grammar point gets its own SRS state (`user.grammar.srs`, lazily created on first exercise). Template choice is weighted by the most-in-need point among a template's `requires`; the tense axis is weighted by the tense point's state; slot fillers strongly prefer Wortschatz words (never-encountered words stay rare at weight 0.2). Grading: a wrong sentence punishes its `points` (requires minus the tense point — that one grades via the verb cell) and the verb cell, but not every word; a correct one rewards words, cell, and points. Correct-before-due reviews leave the schedule untouched so same-day repeats don't compound intervals.
