...
- **Multiple translations:** when one German gloss maps to several Spanish words — a context split (*groß* → alto/grande) or plain synonyms (*lecker* → rico/bueno) — the prompt names the context (*groß (Person)*) and the **intended word is required**. Typing the other valid word is wrong, but the back explains the split with a legend (*groß: alto (Person) · grande (Sache)*) instead of a bare "wrong"; a genuinely wrong answer just gets the correct form. After a correct answer the legend sits under *warum?*. Purely data-driven: adjective senses sharing a gloss base (the gloss stripped of its trailing `(…)` hint) are cross-linked automatically, so the pairing lives in one place. The hint is card-only — the sentence generator strips it, so a template never renders "*ist groß (Sache)*".
